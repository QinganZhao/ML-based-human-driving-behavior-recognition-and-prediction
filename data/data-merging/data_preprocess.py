...
def divide_data(data_matrix, segment_length):
	...
	traj_tensor_1 = np.zeros((segment_length, data_matrix.shape[1], 1))
	traj_tensor_2 = np.zeros((segment_length, data_matrix.shape[1], 1))
	...
	# 			temp_seg_data = np.reshape(temp_seg_data, [temp_seg_data.shape[0], temp_seg_data.shape[1], 1])
	# 			traj_tensor_1 = np.concatenate((traj_tensor_1, temp_seg_data), axis = 2)
	# 	else:
	# 		for j in range(temp_data.shape[0]-segment_length+1):
	# 			temp_seg_data = temp_data[j:j+segment_length, :]
	...
	# 			temp_seg_data = np.reshape(temp_seg_data, [temp_seg_data.shape[0], temp_seg_data.shape[1], 1])
	# 			traj_tensor_2 = np.concatenate((traj_tensor_2, temp_seg_data), axis = 2)
	for i in range(128):
		print('i = '+str(i))
		temp_data = data_matrix[np.where(data_matrix[:,0] == i)]
		if temp_data[0,-1] == 0:
	...
	print (traj_tensor_1.shape)
	print (traj_tensor_2.shape)
	...
	np.save('traj_tensor_1.npy', traj_tensor_1[:,:,1:])
	np.save('traj_tensor_2.npy', traj_tensor_2[:, :, 1:])

data_process()
data_matrix = np.loadtxt('./data_matrix.txt')
divide_data(data_matrix=data_matrix, segment_length=20)
...
#data_matrix = np.loadtxt('./data_matrix.txt')
#xplot_vehicles(case_id=8, data_matrix=data_matrix)

def aggerg_data_process():
	"""
	This function serves to concatenate the information of two cars into one array.
	...
	for i in range(128):
		...
		T = int(car1.shape[0])
		current_data_matrix = np.zeros((T-1,20))
		for j in range(1, T-1):
			...
			current_data_matrix[j, 16] = 10*(car1[j+1, 1] - car1[j, 1])
			current_data_matrix[j, 17] = 10*(car1[j+1, 2] - car1[j, 2])
			current_data_matrix[j, 18] = 10*(car2[j+1, 1] - car2[j, 1])
			current_data_matrix[j, 19] = 10*(car2[j+1, 2] - car2[j, 2])

		current_data_matrix = current_data_matrix[1:, :]
		data_matrix = np.concatenate((data_matrix, current_data_matrix),axis=0)

	np.savetxt('./data_matrix_agg.txt', data_matrix[1:,:],'%.4f')
